...
Every role is pinned to a Claude model tier chosen for task fit: Fable 5 for the roles where errors carry the highest downstream cost (planner, forensic-analyst, advisor — all `xhigh`), Fable 5.1 at `low` effort for the two writable coding roles (coding-worker, fast-coding-worker), Opus 4.8 for high-effort breadth/judgment work (reviewer, edge-case-analyst), Sonnet 5 for execution-heavy work outside the coding path (QA's long tool-call loops, doc-drift's semantic judgment), and Haiku 4.5 for fast, mechanical work. Distribution across the ten roles: **5 Fable · 2 Opus · 2 Sonnet · 1 Haiku**.
...
| `fast-coding-worker` | `fable` | Fable 5.1 (`claude-fable-5-1`) | writable | low |
...
Effort is omitted (`—`) on the sole remaining Haiku role (`helper-worker`) because Haiku does not support the `effort` frontmatter field; its depth is the Haiku tier itself. `doc-reviewer` explicitly sets `medium`, and the two coding roles explicitly set `low`, rather than relying on the `high` default that every other Fable/Opus/Sonnet role would otherwise fall back to.
