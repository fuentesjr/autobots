...
Autobots uses a **task-fit** mapping tuned for quality first and speed second; cost is not a routing input. Two rules follow from the work shape of each role.
...
**Implementers run a big model with low reasoning.** Higher effort makes a model write more code for the same functionality, so the two code-writing roles stay at `low` and get their quality from the model instead: Fable for `coding-worker`, Opus for `fast-coding-worker`, whose shorter turns keep it a distinct fast path. `qa-engineer` is the one writable role at `high`: its writes are throwaway scaffolding, and its reasoning goes into judging what counts as a regression, so it runs on Opus alongside `doc-reviewer`, whose semantic drift checks are judgment work at `high`. Sonnet at `low` is reserved for `helper-worker`, the one role where turn speed is the deliverable. The roster has no Haiku role.
...
| `fast-coding-worker` | `gpt-5.3-codex-spark` | `opus` | Opus 5 |
| `helper-worker` | `gpt-5.4-mini` | `sonnet` | Sonnet 5 |
| `forensic-analyst` | `gpt-5.5` | `fable` | Fable 5.1 |
| `doc-reviewer` | `gpt-5.4-mini` | `opus` | Opus 5 |
| `reviewer` | `gpt-5.5` | `fable` | Fable 5.1 |
| `qa-engineer` | `gpt-5.5` | `opus` | Opus 5 |
| `edge-case-analyst` | `gpt-5.5` | `fable` | Fable 5.1 |
| `advisor` | — (Autobots-only) | `fable` | Fable 5.1 |

Distribution: **6 Fable · 3 Opus · 1 Sonnet · 0 Haiku** (ten roles; `advisor` is an Autobots addition with no Agenticons counterpart — see the advisory pattern in Multi-Agent Patterns).
...
| `fast-coding-worker` | writable | Opus 5 | low | Small localized edits and quick fixes |
...
| `doc-reviewer` | read-only | Opus 5 | high | Documentation correctness and drift review |
| `reviewer` | read-only | Fable 5.1 | high | Standard correctness, security, maintainability, regression review |
| `qa-engineer` | writable | Opus 5 | high | Exploratory QA verification: exercises changes end-to-end, probes regressions, performance, and user-facing rough edges |
| `edge-case-analyst` | read-only | Fable 5.1 | xhigh | Edge-case and coverage-gap discovery: finds unconsidered cases and specifies expected behavior and test cases |
...
| `effort` | Fable/Opus/Sonnet roles | `high`, `xhigh`, etc. — the pinned reasoning effort. Omitted on Haiku roles (unsupported; none currently in the roster). |
...
**Roles.** One writable executor — `coding-worker` (Fable) for normal work, or `fast-coding-worker` (Opus) for the fastest turns — plus the read-only `advisor` role (Fable, `xhigh`).
...
Copying remains the default because it is what most users want: a target repo gets a self-contained snapshot of the package that doesn't change out from under it when the source repo moves on. But the maintainer's own `~/.claude` is a live install of this same repo, kept current by editing the checkout directly rather than reinstalling after every change — the convention there is that each `~/.claude/skills/<name>` is a symlink to a directory whose **top level** contains that skill's `SKILL.md`, not a copy. Autobots' own `SKILL.md` lives one level below the checkout root, at `.claude/skills/autobots/SKILL.md`, so the symlink target must be `.claude/skills/autobots` specifically — linking the checkout root instead would leave `SKILL.md` nested two levels down from where the convention expects it at the top level, which is not a link the convention describes even though it might look superficially similar.

`--symlink` (local-checkout mode only — a symlink needs a real on-disk target, so it errors in remote curl-pipe installs) serves that case: instead of copying, it links `<DEST_ROOT>/skills/autobots` to `<checkout>/.claude/skills/autobots` as a single directory symlink, and links each `<DEST_ROOT>/agents/<name>.md` to the matching file in the checkout. The skill gets a directory-level link (its destination directory belongs entirely to Autobots); the agents directory does not, because it holds files for other agents outside this package, so each agent file is linked individually rather than the whole directory. Both link shapes use absolute targets so they keep resolving regardless of the current working directory. The existing overwrite/`--force`/`--dry-run` semantics (`INS-5`) extend to symlinks unchanged in spirit: unchanged-target symlinks are left alone and reported as skipped, anything else differing is a conflict without `--force`, and `--force` replaces cleanly (removing an existing real directory first, so the new symlink doesn't end up nested inside it) — with a louder warning when the existing destination is a real directory, since `--force` there is a recursive delete, not just a link swap.

Two failure modes drove extra guards beyond the basic link/replace logic above. First, the destination and the symlink target can resolve to the *same* physical path — running `--symlink` from inside the checkout itself with no `--target`, or a `--target` that (under some spelling) points back at the checkout's own `.claude`. Without a check, `--force` would `rm -rf` the very source it's linking from and leave a self-referential dangling link while reporting success; the installer canonicalizes both paths and refuses outright, both up front and per entry. Second, once a skill or agent file has been symlink-installed, a later copy-mode install must not treat that destination as an ordinary file: `cp` follows a live symlink (or the symlinked parent directory, for the skill) and writes through it into whatever it points at, silently modifying the linked checkout instead of the intended destination. Copy-mode installs detect a symlink at the destination (or its parent) — including a dangling one, if the linked checkout has since moved or been deleted — and treat it as its own conflict class, removing the link first under `--force` so the copy lands as a real, decoupled file.
...
5. **Models**: GPT tiers → Claude tiers, task-fit (6 Fable · 3 Opus · 1 Sonnet · 0 Haiku, with the model tier tracking each role's work shape and reasoning effort). Subject to the `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` override caveat.
